...
void MLN::merge() {
  ...
  for (auto i=0; i<this->cliques.size(); i++) {
    int target = -1;
    for (auto j=i+1; j<this->cliques.size(); j++) {
...
void MLN::saveToDotFile(ofstream& file) {
  ...
  for (auto i=0; i<this->cliques.size(); i++) {
    ...
    for (auto i=0; i<literals.size(); i++) {
      string l = literals[i];
      int l_i = node_map[l];
      for (auto j=i+1; j<literals.size(); j++) {
        string r = literals[j];
        int r_i = node_map[r];
